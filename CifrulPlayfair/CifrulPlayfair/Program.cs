...
namespace CifrulPlayfair
{
    internal class Program
    {
        ...
        {
            string alfabet = "ABCDEFGHIKLMNOPQRSTUVWXYZ"; // Fără J
            bool[] used = new bool[26]; // Marcăm literele deja folosite
            ...
            foreach (char c in cheia.ToUpper())
            {
                ...
                if (!used[modificat - 'A']) // Verificăm dacă litera nu a fost deja folosită
                {
        ...
        public static (int, int) GăseștePoziția(char c, char[,] matrice)
        {
            ...
            return (-1, -1); // Dacă nu găsim litera
        }
